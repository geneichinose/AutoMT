...
void setWeekOfYear( MyTime *t )
{
	...
	if(verbose) fprintf( stderr, "setWeekOfYear(): \n");
...
/****************************************************************************/
/*** if time t is beween t0 and t1 then return 1 else return 0 ***/
/****************************************************************************/
int mytime_between( MyTime *t0, MyTime *t1, MyTime *t )
{
	if( (t->epoch < t1->epoch) && (t->epoch > t0->epoch) )
		return 1;
	else
		return 0;
}
...
/*** given year and week of year return the time structure ***/

void setTime_from_WeekofYear( MyTime *t, int yr, int wofy )
{
        void initialize_mytime( MyTime *a );
        void complete( MyTime *a );
        void jday_to_month( MyTime *t );
...
{
	sprintf( str, "%4d-%02d-%02dT%02d:%02d:%05.2f",
                t->year,
                t->month,
                t->mday,
                t->hour,
